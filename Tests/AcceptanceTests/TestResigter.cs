...
namespace Tests.AcceptanceTests
{
    ...
    public class TestRegister
    {
        ...
        [Test]
        public async Task TestSuccess(string username, string email, string name, string birthday, string address, string password)
        {
        ...
        [Test]
        public async Task TestFailureInput(string username, string email, string name, string birthday, string address, string password)
        {
            MemberInfo info = new MemberInfo(username, email, name, DateTime.ParseExact(birthday, "dd/MM/yyyy", CultureInfo.InvariantCulture), address);
            string token = _auth.Connect();
            Result regResult = await _auth.Register(token, info, password);
        ...
        [TestCase("Yossi11","yossi@gmail.com", "Yossi Park", "19/04/2005", "hazait 14", "tdddev123")]
        [Test]
        public async Task TestFailureLogic(string username, string email, string name, string birthday, string address, string password)
        {
            MemberInfo info = new MemberInfo(username, email, name, DateTime.ParseExact(birthday, "dd/MM/yyyy", CultureInfo.InvariantCulture), address);
            string token = _auth.Connect();
